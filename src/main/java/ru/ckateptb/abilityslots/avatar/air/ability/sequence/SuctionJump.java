...
@Sequence({
    ...
    @AbilityAction(ability = AirSuction.class, action = SequenceAction.SNEAK_RELEASE)
})
public class SuctionJump extends Ability {
	 ...
	 private static double force = 2;
		...
		public ActivateResult activate(ActivationMethod paramActivationMethod) {
			...
			livingEntity.setVelocity(livingEntity.getLocation().getDirection().multiply(-1).normalize().multiply(force));
